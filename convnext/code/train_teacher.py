...
def train_one_fold(fold, train_loader, test_loader):
    ...
    if default_configs["use_arcface"]:
        criterion_class = ArcFaceLossAdaptiveMargin(m=default_configs["arcface_m"], s=default_configs["arcface_s"])
    else:
    ...
    # criterion_class = FocalCosineLoss()
...
#     count_parameters(model.src_embed)
    ...
    if weights_path[fold] is not None:
        pretrained_dict = torch.load(weights_path[fold])
    ...
    if APEX_AVAILABLE:
        model, optimizer_model = amp.initialize(
            model, optimizer_model, opt_level="O2", loss_scale="dynamic"
        )
    ...
    for epoch in range(start_epoch, default_configs["num_epoch"]):
        ...
        # print("\n=============================================================================")
        if epoch >= default_configs['start_ema_epoch']:
            val_metric, ground_truths, scores = eval(test_loader, model_ema.ema, criterion_class, device, default_configs["n_eval"], fold, epoch, True, default_configs)
            for val_metric_type in val_metric_type_list:
